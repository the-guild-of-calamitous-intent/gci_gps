...
TEST(gps, dummy) { EXPECT_TRUE(true); }
// Test fixture for common setup
class NMEATest : public ::testing::Test {
protected:
  void SetUp() override {
    memset(&gga_data, 0, sizeof(gga_t));
    memset(&rmc_data, 0, sizeof(rmc_t));
    memset(&gsa_data, 0, sizeof(gsa_t));
    memset(&gps_time, 0, sizeof(gps_time_t));
    // memset(&pos_error, 0, sizeof(PositionError));
  }
...
// Test parse_gga
TEST_F(NMEATest, ParseGGA_ValidMessage) {
  const char *msg =
  ...
  EXPECT_FLOAT_EQ(0.9, gga_data.hdop);
  EXPECT_FLOAT_EQ(545.4, gga_data.altitude);
  EXPECT_EQ('M', gga_data.alt_unit);
  EXPECT_FLOAT_EQ(46.9, gga_data.geoid_separation);
  EXPECT_EQ('M', gga_data.geoid_unit);
}

TEST_F(NMEATest, ParseGGA_EmptyFields) {
  ...
  EXPECT_FLOAT_EQ(99.9, gga_data.hdop);
  EXPECT_FLOAT_EQ(0.0, gga_data.altitude);
  EXPECT_EQ('M', gga_data.alt_unit);
  EXPECT_FLOAT_EQ(0.0, gga_data.geoid_separation);
  EXPECT_EQ('M', gga_data.geoid_unit);
}

TEST_F(NMEATest, ParseGGA_GOOD_2) {
  ...
  EXPECT_EQ('N', gga_data.lat_dir);
  EXPECT_FLOAT_EQ(-121.31432683, (float)gga_data.longitude);
  EXPECT_EQ('W', gga_data.lon_dir);
  EXPECT_EQ(1, gga_data.fix_quality);
  EXPECT_EQ(12, gga_data.satellites);
  EXPECT_FLOAT_EQ(0.98, gga_data.hdop);
  EXPECT_FLOAT_EQ(1113.0, gga_data.altitude);
  EXPECT_EQ('M', gga_data.alt_unit);
  EXPECT_FLOAT_EQ(-21.3, gga_data.geoid_separation);
  EXPECT_EQ('M', gga_data.geoid_unit);
}

TEST_F(NMEATest, ParseGGA_InvalidMessage) {
  const char *msg = "$GPXXX,123519,,,,,0,00,99.9,,,M,,M,,*56";
  EXPECT_EQ(NEMA_INVALID_MSG_ID, parse_gga(msg, &gga_data));
}

TEST_F(NMEATest, ParseGGA_IncompleteMessage) {
  const char *msg = "$GPGGA,123519,4807.038,N,01131.000,E,1,08*47";
  EXPECT_EQ(NEMA_INCOMPLETE_MSG, parse_gga(msg, &gga_data));
}

// Test parse_rmc
TEST_F(NMEATest, ParseRMC_ValidMessage) {
  const char *msg =
  ...
  EXPECT_FLOAT_EQ(22.4, rmc_data.speed);
  EXPECT_FLOAT_EQ(84.4, rmc_data.course);
  EXPECT_STREQ("230394", rmc_data.date);
  EXPECT_FLOAT_EQ(-3.1, rmc_data.mag_var);
  EXPECT_EQ('W', rmc_data.mag_var_dir);
}

TEST_F(NMEATest, ParseRMC_EmptyFields) {
...
TEST_F(NMEATest, ParseGSA_ValidMessage) {
  ...
  EXPECT_FLOAT_EQ(1.7, gsa_data.pdop);
  EXPECT_FLOAT_EQ(1.0, gsa_data.hdop);
  EXPECT_FLOAT_EQ(1.3, gsa_data.vdop);
}

TEST_F(NMEATest, ParseGSA_EmptyFields) {
  ...
  EXPECT_FLOAT_EQ(1.0, gsa_data.pdop);
  EXPECT_FLOAT_EQ(0.8, gsa_data.hdop);
  EXPECT_FLOAT_EQ(0.6, gsa_data.vdop);
}

TEST_F(NMEATest, ParseGSA_InvalidMessage) {
  const char *msg = "$GPXXX,A,3,19,28,14,18,,,,,,,1.7,1.0,1.3*35";
  EXPECT_EQ(NEMA_INVALID_MSG_ID, parse_gsa(msg, &gsa_data));
}

TEST_F(NMEATest, ParseGSA_IncompleteMessage) {
  const char *msg = "$GPGSA,A,3,19,28,14,18,,,,,,,1.7*35";
  EXPECT_EQ(NEMA_INCOMPLETE_MSG, parse_gsa(msg, &gsa_data));
}

// Test parse_gps_time
TEST_F(NMEATest, ParseGPSTime_ValidWithMilliseconds) {
...
TEST_F(NMEATest, ParseGPSTime_ValidWithoutMilliseconds) {
